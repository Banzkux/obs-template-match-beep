...
template<typename T> bool between(const T &value, const T &min, const T &max)
{
	LVK_ASSERT(min <= max);
...
FrameIngest::FrameIngest(video_format format) : m_Format(format)
{
...
void I4XXIngest::upload(const obs_source_frame *src, cv::UMat &dst)
{
	...
	if (chroma_size != frame_size) {
		...
		merge_planes(y_roi, u_roi, v_roi, dst);
}
...
void I4XXIngest::download(const cv::UMat &src, obs_source_frame *dst)
{
	...
	if (m_ChromaScaling.width != 1.0f || m_ChromaScaling.height != 1.0f) {
		...
		download_planes(m_YPlane, m_UPlane, m_VPlane, frame);
}

//---------------------------------------------------------------------------------------------------------------------

NV12Ingest::NV12Ingest() : FrameIngest(VIDEO_FORMAT_NV12) {}
...
P444Ingest::P444Ingest() : FrameIngest(VIDEO_FORMAT_AYUV) {}
...
void P444Ingest::download(const cv::UMat &src, obs_source_frame *dst)
{
	...
	cv::mixChannels({{src, dst_roi}}, std::vector<cv::UMat>{m_MixBuffer},
			{3, 0, 0, 1, 1, 2, 2, 3});
